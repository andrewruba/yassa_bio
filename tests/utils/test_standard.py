...
class TestSeriesConcentrationMap:
    ...
    def test_single_step_dilution_factor(self):
        ser = StandardSeries(start_concentration=60, dilution_factor=3, num_levels=3)
        # 60, 20, 6.666…
    ...
    def test_float_dilution_factor(self):
        ser = StandardSeries(
            start_concentration=1_000, dilution_factor=1.5, num_levels=3
        )
    ...
    def test_num_levels_respected(self):
        ser = StandardSeries(start_concentration=10, dilution_factor=2, num_levels=2)
        result = series_concentration_map(ser)
        assert set(result.keys()) == {1, 2}

    def test_mapping_is_monotonically_decreasing(self):
        ser = StandardSeries(start_concentration=256, dilution_factor=2, num_levels=5)
